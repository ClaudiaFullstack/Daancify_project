...
controller.getUsers = (_req, res) => {
    let sql = "SELECT * FROM user WHERE logical_erase = 0";
...
controller.getTeachers = (_req, res) => {
    let sql = "SELECT * FROM user WHERE logical_erase = 0 AND user_type = 2";
...
// CREAMOS UN NUEVO USUARIO con img
// SAVE USER 
...
//     console.log(req.body);
//         connection.query(sqlUser,{name,user_name,last_name,email,phone,password,user_type,avatar}, (error, userResult) => {
//             console.log(userResult.insertId);
//             let user_id = userResult.insertId;
